...
function e_or_d_change(radio) {
	...
	enc_or_dec_header.innerHTML = "What kind of " + radio.id + "ion would you like to do?";
}
...
function validateUsername(username) {
	...
	if (username.length < min_username_len) {
		return "Username needs to be longer than " + min_username_len + " characters\n";
...
function validatePassword(password) {
	...
	if (password.length < min_password_len) {
		return "Password needs to be longer than " + min_password_len + " characters.\n";
...
function decrypt_text(tr) {
	...
	var message = encryption_type + " for ID " + cells[0].innerHTML + ": " + decrypted_text;
...
function decrypt_double_trans(key1, key2, text) {
	...
	// console.log("Shuffled: " + shuffled_text);
	...
	for(var i = 0; i < key_1_arr.length; i++) {
		...
		for(var j = 0; j < key_2_arr.length; j++) {
			...
			// console.log("Shuff_row: " + shuffled_text[13]);
			// console.log("Row: " + shuff_row + ", Col: " + shuff_col + ", Char: " + shuffled_text
				// [shuff_row]
				// [shuff_col]);
